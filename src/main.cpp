...
String correctTags [NUM_READERS] = {"12A36F1E"}; // Correct RFID tag UIDs
...
void setup()
{
  ...
  me.init("Claremont-ETC", "Cl@remontI0T", "mqtt://broker.hivemq.com", "Clam 3", callbacks, variables);
}
...
void loop()
{
  while (tagMatch() | checkIDs()) {
    ...
    if (tagMatch()) {
      ...
      me.call_remote_fn("clam", "clam 3 correct");
    } else {
      if(DEBUG) {
        Serial.print("Incorrect tags!\n");
      }
      me.call_remote_fn("clam", "clam 3 incorrect");
    }
